...
#include "sensor.h"
#include "logger.h"
#include "user-sensor-vu13p-qsfp.h"
#include "user-payload-vu13p-qsfp.h"
#include "semaphore.h"
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
...
/*==============================================================*/
void temp_qsfp_max_poll( unsigned char *arg );
...
void user_module_sensor_init(void) {
	...
	/* 		All Sensors											*/
	/*==============================================================*/
  sd[4].scan_function = read_sensor_temp_qsfp_max;
}
...
void read_sensor_temp_qsfp_max(void) {
    ...
    if (check_power_up()) {
				...
				for (int i = 0; i < 30; i++)
				{
						...
						i2c_write_qsfp2 (i2c_fd_snsr[i2c_ch], exp0, er[0]);
						i2c_write_qsfp2 (i2c_fd_snsr[i2c_ch], exp1, er[1]);
...
void temp_qsfp_max_poll( unsigned char *arg ) {
        unsigned char temp_qsfp_max_timer_handle;

        read_sensor_temp_qsfp_max();

        // Re-start the timer
        timer_add_callout_queue( (void *)&temp_qsfp_max_timer_handle,
                        5*SEC, temp_qsfp_max_poll, 0 ); /* 5 sec timeout */
}
